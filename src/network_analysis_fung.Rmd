...
```{r}
egde_threshold_never = quantile(edge_weights_never_df$weight, 0.99)
egde_threshold_recent = quantile(edge_weights_recent_df$weight, 0.99)
egde_threshold_past = quantile(edge_weights_past_df$weight, 0.99)
...
ggsave("plot_circular/fungi/never_glasso.pdf", plot.never.glasso, width = 10, height = 14, dpi = 600, device = cairo_pdf, bg = "white", limitsize = FALSE)
```
...
ggsave("plot_circular/fungi/recent_glasso.pdf", plot.recent.glasso, width = 10, height = 14, dpi = 600, device = cairo_pdf, bg = "white", limitsize = FALSE)
```
...
ggsave("plot_circular/fungi/past_glasso.pdf", plot.past.glasso, width = 10, height = 14, dpi = 600, device = cairo_pdf, bg = "white", limitsize = FALSE)
```
...
```{r}
# Function for fungal networks with selective node labeling and edge filtering
plot_fungal_unique_networks = function(net1, net2, net3, method_names, title, phylum_mapping, node_size = 10,
                                      edge_label_threshold = 0.995, edge_display_threshold = 0.95) {
  # Get edge lists and standardize them
  edges1 = get.edgelist(net1)
  edges2 = get.edgelist(net2)
  edges3 = get.edgelist(net3)
  ...
  for (i in 1:3) {
    ...
    # Initialize all edges as included (for node identification)
    all_edges_df = unique_edges_df
    
    # Get the weight thresholds for edge display and node labeling
    display_threshold = quantile(unique_edges_df$abs_weight, edge_display_threshold)
    label_threshold = quantile(unique_edges_df$abs_weight, edge_label_threshold)
    
    # Create a new graph with only the unique edges but keep all nodes
    all_nodes = unique(c(all_edges_df$V1, all_edges_df$V2))
    
    # Create an empty graph
    unique_graph = graph.empty(directed = FALSE, n = length(all_nodes))
    V(unique_graph)$name = all_nodes
    
    # Filter edges for display (but keep all nodes)
    visible_edges_df = unique_edges_df[unique_edges_df$abs_weight > display_threshold, ]
    
    # Add the visible edges with weights
    for (j in 1:nrow(visible_edges_df)) {
      unique_graph = add_edges(
        unique_graph, 
        c(visible_edges_df$V1[j], visible_edges_df$V2[j]),
        weight = visible_edges_df$weight[j],
        abs_weight = visible_edges_df$abs_weight[j]
      )
    }
    ...
    # Identify nodes connected to high-weight edges for labeling
    nodes_to_label = c()
    if (ecount(unique_graph) > 0) {  # Check if there are any edges
    ...
    print(paste("Network for", method_names[i], "fumigation:"))
    print(paste("Number of nodes:", length(V(unique_graph))))
    print(paste("Number of nodes with labels:", sum(V(unique_graph)$show_label)))
    print(paste("Edge display threshold:", display_threshold))
    print(paste("Edge label threshold:", label_threshold))
    
    if (ecount(unique_graph) > 0) {
      # Assign edge colors
      E(unique_graph)$edge_color = ifelse(E(unique_graph)$weight > 0, "#BBBBBB", "#D2B48C")
      
      print(paste("Number of visible edges:", ecount(unique_graph)))
      ...
      print("No edges meet the display threshold")
      next
    }
...
unique_result = plot_fungal_unique_networks(
  never.glasso, recent.glasso, past.glasso,
  c("Never", "Recent", "Past"),
...
```{r}
write.csv(top_20, "plot_circular/fungi/top_20.csv")
```
...
Within the top 20 taxa, we can classify them into 5 groups:
1. Continuous decline: Abundance decreases after fumigation and continues to decrease
2. Decline then strong recovery: Abundance drops after fumigation but recovers to levels higher than pre-fumigation
3. Decline then partial recovery: Abundance drops after fumigation and shows some recovery but stays below pre-fumigation levels
4. Continuous increase: Abundance increases after fumigation and continues to increase
5. Spike then decline: Abundance increases after fumigation but then decreases
...
# Function to classify abundance patterns
classify_pattern = function(d1, d2, d3) {
  if (d2 < d1) {  # Decrease after fumigation
    if (d3 < d2) {  # Continues to decrease
      return(1)  # "Continuous decline"
    } else if (d3 > d2) {  # Shows recovery
      if (d3 > d1) {
        return(2)  # "Decline then strong recovery"
      } else {
        return(3)  # "Decline then partial recovery"
      }
    }
  } else {  # Increase after fumigation
    if (d3 > d2) {
      return(4)  # "Continuous increase"
    } else {
      return(5)  # "Spike then decline"
    }
  }
  return(NA)  # For any other patterns
}

# Calculate abundances and classify patterns
for (name in top_20$names) {
  # Calculate mean abundance for each period
  d1 = sum(never_mat[,name]) / length(never_fumigated)    # never
  d2 = sum(recent_mat[,name]) / length(recent_fumigated)  # recent
  d3 = sum(past_mat[,name]) / length(past_fumigated)      # past
  ...
  # Classify pattern
  group_df[group_df$names == name,]$group = classify_pattern(d1, d2, d3)
}

# Add descriptive labels
group_df$pattern = factor(group_df$group,
                         levels = 1:5,
                         labels = c("Continuous decline",
                                  "Decline then strong recovery",
                                  "Decline then partial recovery",
                                  "Continuous increase",
                                  "Spike then decline"))

# Display results with pattern descriptions
group_df
```

Write group_df to csv

```{r}
write.csv(group_df, "plot_circular/fungi/group_df.csv")
```
