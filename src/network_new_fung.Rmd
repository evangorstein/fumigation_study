...
plot_circular_network = function(net, title, phylum_mapping,degree_filter, node_size = 2, edge_threshold = 0.07){
  # filter out edges
    net_filtered = delete_edges(net, which(E(net)$weight <= edge_threshold))
    
    phylum_colors = pal_npg("nrc")(8)
   # phylum_colors = colorRampPalette(base_colors)(27)  # Repeat for 27 phyla
    ...
    geom_node_text(aes(filter = size > 999, label = name, size = 3), family = "serif", repel = TRUE, show.legend = FALSE) +
...
quantile(all_edge_weights, c(0.75, 0.8, 0.85, 0.9))
```
...
plot_common_edges_network = function(net1, net2, net3, title, phylum_mapping, node_size = 10, edge_threshold = 0.07) {
  ...
  unique_phylums_num = length(unique(V(common_graph)$phylum))
...
```{r}
# save plot
fungi_top10_plot = plot(response10.horse) + 
  theme(
    legend.title = element_text(size = 22),    # Legend title size
    legend.text = element_text(size = 20),     # Legend text size
    legend.key.size = unit(1.5, 'cm')          
...
```{r}
fungi_top10_single_plot = plot(response10_single.horse) + 
  theme(
    legend.title = element_text(size = 22),    # Legend title size
    legend.text = element_text(size = 20),     # Legend text size
    legend.key.size = unit(1.5, 'cm')          
