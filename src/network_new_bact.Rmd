...
plot_circular_network = function(net, title, phylum_mapping,degree_filter, node_size = 3, edge_threshold = 0.07){
  # filter out edges
    ...
    node1_phylum = V(net_filtered)$phylum[edge_ends[1]]
    phylum_index = match(node1_phylum, unique(V(net_filtered)$phylum))
    if(is.na(phylum_index)) phylum_index = 1
    return(scales::muted(phylum_colors[phylum_index]))
  })
