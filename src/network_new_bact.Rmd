...
```{r}
# # Helper function to compute a "clock" layout
# custom_clock_layout <- function(net, group_attr = "phylum", radius = 1, start_angle = 0) {
#   # Extract vertex data from the graph
#   v_df <- as_data_frame(net, what = "vertices")
#   v_names <- v_df$name
  
#   # Get unique groups (phylum) in sorted order; you can change this order if needed
#   groups <- sort(unique(v_df[[group_attr]]))
#   n_groups <- length(groups)
  
#   # Initialize a matrix to store coordinates for each vertex
#   coords <- matrix(NA, nrow = vcount(net), ncol = 2)
  
#   # For each group, assign an arc (clock segment)
#   for (i in seq_along(groups)) {
#     group <- groups[i]
#     # Find indices of vertices in this group
#     indices <- which(v_df[[group_attr]] == group)
#     k <- length(indices)
    
#     # Define the segment boundaries (in radians)
#     seg_start <- start_angle + (i - 1) * (2 * pi / n_groups)
#     seg_end   <- start_angle + i * (2 * pi / n_groups)
    
#     # Evenly distribute nodes along the arc
#     angles <- if (k == 1) {
#       (seg_start + seg_end) / 2
#     } else {
#       seq(seg_start, seg_end, length.out = k + 2)[-c(1, k + 2)]
#     }
    
#     # Assign coordinates using polar-to-cartesian conversion
#     coords[indices, 1] <- radius * cos(angles)
#     coords[indices, 2] <- radius * sin(angles)
#   }
  
#   # Create a layout data frame with computed coordinates and vertex names
#   layout_df <- data.frame(name = v_names, x = coords[, 1], y = coords[, 2])
#   # Merge the computed coordinates with the vertex attributes
#   layout_df <- merge(layout_df, v_df, by = "name", sort = FALSE)
  
#   return(layout_df)
# }

# # Modified plotting function using the custom clock layout
# plot_circular_network <- function(net, title, phylum_mapping, degree_filter, 
#                                   node_size = 3, edge_threshold = 0.07, radius = 1) {
#   # Remove weak edges based on the provided threshold
#   net_filtered <- delete_edges(net, which(E(net)$abs_weight <= edge_threshold))
  
#   # Ensure that each vertex has a 'size' attribute (here using degree)
#   V(net_filtered)$size <- degree(net_filtered)
  
#   # Set up colors for up to 27 phyla
#   base_colors <- pal_npg("nrc")(10)
#   phylum_colors <- colorRampPalette(base_colors)(27)
  
#   # Add phylum information to each vertex (using your mapping)
#   V(net_filtered)$phylum <- phylum_mapping[V(net_filtered)$name]
#   V(net_filtered)$phylum[is.na(V(net_filtered)$phylum)] <- "Unknown_Phylum"
  
#   # Compute the manual "clock" layout (each phylum gets its clock segment)
#   layout_df <- custom_clock_layout(net_filtered, group_attr = "phylum", radius = radius)
  
#   # Set edge color based on the sign of the weight and scale alpha by absolute weight
...
#   # Build the plot using the manual layout
#   network <- ggraph(net_filtered, layout = "manual", node.positions = layout_df) +
...
#     geom_node_text(aes(filter = size > degree_filter, label = name), 
...
plot_circular_network = function(net, title, phylum_mapping,degree_filter, node_size = 3, edge_threshold = 0.07){
  # filter out edges
    ...
    # phylum_order = order(V(net_filtered)$phylum)
    # net_filtered = permute(net_filtered, phylum_order)
    # layout_coords = igraph::layout_in_circle(net_filtered)

    # layout_df = as.data.frame(layout_coords)
    # colnames(layout_df) = c("x", "y")
    # layout_df$name = V(net_filtered)$name
    

    # add edge color with more contrasting colors
    # Light silver-gray for positive (#E0E0E0), darker tan/sand for negative (#D2B48C)
    E(net_filtered)$edge_color = ifelse(E(net_filtered)$weight >= 0, "#E0E0E0", "#D2B48C") 
    # assign edge alpha
    E(net_filtered)$edge_alpha = scales::rescale(E(net_filtered)$abs_weight, to = c(0.3, 0.8))
...
plot_unique_edges_network = function(net1, net2, net3, method_names, title, phylum_mapping, node_size = 10) {
  ...
  for (i in 1:3) {
    ...
    # Assign edge colors
    E(unique_graph)$edge_color = ifelse(E(unique_graph)$weight > 0, "#E0E0E0", "#D2B48C")
