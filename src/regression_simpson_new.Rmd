...
label_data_treatment_b$max_y <- label_data_treatment_b$max_y * 1.05

# Creating the plot
p_treatment_b <- ggplot(sample_metadata_treat, aes(x=Treatment, y=inv_simpson)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title="Alpha-Diversity (inv_simpson) by Treatment",
       x="Treatment", y="Alpha-Diversity (inv_simpson)") +
...
labels_fumigation_f <- c("never" = "a",  # Base group
            ...
            "recent" = "a") # Significantly different from base
...
label_data_fumigation_f$max_y <- label_data_fumigation_f$max_y * 1.05

# Creating the plot
p_fumigation_f <- ggplot(sample_metadata, aes(x=fumigation, y=inv_simpson_fungi)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title="Alpha-Diversity (inv_simpson) for Fungi by Fumigation Status",
       x="Fumigation Status", y="Alpha-Diversity (inv_simpson)") +
...
```{r}
unique_treatments = unique(sample_metadata$Treatment)
...
group_max_treatment_f = sample_metadata %>%
  group_by(Treatment) %>%
  summarise(max_y = max(inv_simpson_fungi, na.rm = TRUE)) %>%
  ungroup()
...
label_data_treatment_f$max_y <- label_data_treatment_f$max_y * 1.05

# Creating the plot
p_treatment_f <- ggplot(sample_metadata, aes(x=Treatment, y=inv_simpson_fungi)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title="Alpha-Diversity (inv_simpson) for Fungi by Treatment",
       x="Treatment", y="Alpha-Diversity (inv_simpson)") +
...
```{r}
model_t = lm(inv_simpson_fungi ~ Treatment, data = sample_metadata)
summary(model_t)
```
...
# then, construct a linear model with only treatment
model_treat = lm(inv_simpson_fungi ~ Treatment, data = sample_metadata)
