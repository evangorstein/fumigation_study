...
labels_fumigation_b <- c("never" = "b",  # Base group
...
```{r}
# save the plot
ggsave("simpson_fumigation_bacteria.pdf", p_fumigation_b)
```
...
```{r}
# save the plot
ggsave("simpson_treatment_bacteria.pdf", p_treatment_b)
```
...
```{r}
# save the plot
ggsave("simpson_fumigation_fungi.pdf", p_fumigation_f)
```
...
```{r}
# anova for fumigation status
anova_result = aov(inv_simpson_fungi ~ fumigation, data = sample_metadata_treat)
anova_result
```

```{r}
#TukeyHSD for fumigation status
tukey_result = TukeyHSD(sample_metadata_treat)
tukey_result
```
...
```{r}
# save the plot
ggsave("simpson_treatment_fungi.pdf", p_treatment_f)
```
